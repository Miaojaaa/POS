...
type Product = { id: string; name: string; unitVolumeMg: number; costPerUnit: number; reorderPoint: number; mainStock?: { quantity: number }; subStock?: { quantity: number; currentVolumeMg: number } };

export default function ProductsPage() {
  const [products, setProducts] = useState<Product[]>([]);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ name: "", unitVolumeMg: "", costPerUnit: "", reorderPoint: "", initialMain: "10", initialSub: "2" });
  ...
  async function save() {
    if (!form.name || !form.unitVolumeMg || !form.costPerUnit) { alert("กรุณากรอกข้อมูลให้ครบ"); return; }
    await fetch("/api/products", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: form.name,
        unitVolumeMg: Number(form.unitVolumeMg),
          ...
          <tbody>
            {products.map(p => (
              <tr key={p.id} style={{ borderBottom: "1px solid #f5f5f5" }}>
                ...
                <td style={{ padding: "8px 12px", textAlign: "center" }}>{(p.unitVolumeMg / 1000).toFixed(0)} กรัม</td>
        ...
        <div className="modal-overlay">
          <div className="modal">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              ...
              <div><label className="label">ปริมาณต่อขวด (มิลลิกรัม)</label><input type="number" className="input" value={form.unitVolumeMg} onChange={e => setForm({ ...form, unitVolumeMg: e.target.value })} placeholder="500000 = 500g" /></div>
              ...
              <div><label className="label">Reorder Point (มก.)</label><input type="number" className="input" value={form.reorderPoint} onChange={e => setForm({ ...form, reorderPoint: e.target.value })} placeholder="1000000 = 1kg" /></div>
