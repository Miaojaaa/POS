...
export default async function DashboardPage() {
  ...
  const todayRevenue = todayOrders.filter(o => o.status === "DONE").reduce((sum, o) => sum + o.total, 0);
  const monthRevenue = monthOrders.reduce((sum, o) => sum + o.total, 0);
  const todayCount = todayOrders.filter(o => o.status === "DONE").length;

  const cards = [
    { label: "ยอดขายวันนี้", value: `฿${todayRevenue.toLocaleString()}`, icon: "💰", color: "#6B7C45" },
    { label: "ยอดขายเดือนนี้", value: `฿${monthRevenue.toLocaleString()}`, icon: "📈", color: "#8FA65A" },
    { label: "ออร์เดอร์วันนี้", value: `${todayCount} ออร์เดอร์`, icon: "📋", color: "#5A7CA6" },
    { label: "คิวที่รออยู่", value: `${queueCount} คิว`, icon: "⏳", color: "#C4863B" },
    { label: "สมาชิกทั้งหมด", value: `${memberCount} คน`, icon: "👥", color: "#A65A7C" },
  ];
                  ...
                  <tr key={o.id} style={{ borderBottom: "1px solid #f5f5f5" }}>
                    ...
                    <td style={{ padding: "4px 8px", textAlign: "center" }}>
                      <span className={`badge badge-${o.status.toLowerCase().replace("_", "-")}`}>
                        {o.status === "WAITING" ? "รอ" : o.status === "IN_PROGRESS" ? "กำลังทำ" : o.status === "DONE" ? "เสร็จ" : "ยกเลิก"}
                      </span>
                    </td>
                  </tr>
                ))}
