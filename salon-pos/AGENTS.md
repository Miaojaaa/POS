...
- Multi-dev sync uses two snapshots in `prisma/` (both committed; `dev.db` itself is git-ignored):
  ...
  - A pre-commit hook (`.githooks/pre-commit`, wired up by `npm install` → `scripts/setup-hooks.js`) runs both `check-config-sync.js` and `check-catalog-sync.js` and blocks any commit whose `dev.db` has drifted from either snapshot. See `.agent/skills/shared-config-sync/SKILL.md` for the full workflow, diagnostic checklist, and anti-patterns.
